...
class OrchestratorAgent(BaseAgent):
    ...
    def run(self, context: RuntimeContext) -> AgentResult:
        ...
        thought = self.thought(
            ...
            reasoning=f'{ORCHESTRATOR_PROMPT} Objective analyzed with complexity={profile.complexity_score}; selecting {workflow_mode} mode.',
        )
        action = self.action(
            'set_workflow',
            {
        ...
        context.remember('objective_profile', {
            'complexity_score': profile.complexity_score,
            'urgency_score': profile.urgency_score,
        ...
        return AgentResult(
            agent=self.role,
            status='ok',
            output={'coordinated': True, 'workflow_mode': workflow_mode},
            thoughts=[thought],
            actions=[action],
            messages=[message],
        )
